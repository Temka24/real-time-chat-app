...
const Signup: React.FC = () => {
    ...
    const avatarUrls: string[] = ["/avatar-1.png", "/avatar-2.png", "/avatar-3.png", "/avatar-4.png"]

    const [isOpenSetAvatar, setIsOpenSetAvatar] = useState<boolean>(false)
    const [formData, setFormData] = useState<{ username: string; email: string; password: string; avatarImage: string }>({ username: "", email: "", password: "", avatarImage: "" })
    ...
    const handleOpenAvatar = (e: React.FormEvent) => {
        ...
        if (!formData.username || !formData.email || !formData.password) {
            toast.error(<div className={`${akaya.className} text-[20px]`}>Please fill the all field</div>)
            return;
        }
            ...
            <div className='w-screen pb-[50px] bg-[#0b091a] py-[100px] h-screen max-h-[1000px]'>
                {
                    isOpenSetAvatar ? (
                        <div className='font-[500] px-[100px] mx-auto w-[450px] py-[50px] mt-[100px] relative rounded-[30px] flex flex-col items-center justify-center gap-[40px]'>
                            <p className='text-nowrap text-[24px]'>Pick an avatar as your profile picture</p>
                            <div className='flex flex-row items-center justify-center gap-[40px]'>
                                {
                                    avatarUrls.map((avtar, i) =>
                                        <div key={i} onClick={() => setFormData({ ...formData, avatarImage: avtar })} className={`h-[80px] w-[80px] ${formData.avatarImage === avtar && "border border-[5px] border-pink-500 rounded-full"}`}>
                                            <Image
                                                src={avtar}
                                                alt="logo"
                                                height={100}
                        ...
                        <div className='bg-black font-[500] px-[100px] mx-auto w-[450px] py-[50px] relative rounded-[30px] flex flex-col items-center justify-center gap-[30px]'>
                            ...
                            <form>
                                ...
                                <input
                                    ...
                                    className="bg-transparent border border-[2px] mt-[20px] border-[#2a1454] focus:border-pink-500/70 focus:outline focus:outline-pink-500/70 placeholder:text-joke/50 text-joke rounded-[6px] px-[20px] py-[8px] w-[100%]"
                                />
                                <div onClick={() => setPass_1_visible(!pass_1_visible)} className='translate-x-[210px] translate-y-[-35px] text-pink-500 cursor-pointer absolute'>
                                ...
                                <input
                                    ...
                                    className="bg-transparent border border-[2px] mt-[20px] border-[#2a1454] focus:border-pink-500/70 focus:outline focus:outline-pink-500/70 placeholder:text-joke/50 text-joke rounded-[6px] px-[20px] py-[8px] w-[100%]"
                                />
                                <div onClick={() => setPass_2_visible(!pass_2_visible)} className='translate-x-[210px] translate-y-[-35px] text-pink-500 cursor-pointer absolute'>
                            ...
                            <div className={`${akaya.className} text-nowrap text-[20px]`}>
                                Already have an account ? <span onClick={() => router.push("/login")} className='text-joke cursor-pointer'> Login</span>
                            </div>
                        </div>
                    )
                }
